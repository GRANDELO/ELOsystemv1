...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://elosystemv1.onrender.com/api/delivery/reset-password', formData);
  ...
  useEffect(() => {
    const validateStep = () => {
      switch (currentStep) {
        case 1:
          const emailValid = /^[a-zA-Z0-9!#$%&'*+/=?^_`{|}~.-]{1,64}@[a-zA-Z0-9.-]{1,255}\.[a-zA-Z]{2,}$/.test(formData.email);
          return emailValid;
        case 2:
